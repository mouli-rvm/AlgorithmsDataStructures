...
def findCrossoverIndexHelper(x, y, left, right):
    ...
    # your code here 𝑦𝑖≤𝑥𝑖   and 𝑦𝑖+1>𝑥𝑖+1
    mid = (left + right) // 2

    if(y[mid] <= x[mid] and y[mid+1] > x[mid+1]):
        return mid
    elif(y[mid] <= x[mid]):
        return findCrossoverIndexHelper(x, y, mid + 1, right)    #left = mid + 1
    else:
        return findCrossoverIndexHelper(x, y, left, mid - 1)    #right = mid - 1
